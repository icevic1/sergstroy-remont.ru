...
class MY_Form_validation extends CI_Form_validation 
{
	...
	{
		$CI =& get_instance();
		$CI->form_validation->set_message('date', 'The date format is incorrect.');
	
		$exploded = explode("-", $value);
		if (count($exploded) != 3) return false;
		list($year, $month, $date) = explode("-", $value);
	
		if (is_numeric($year)&&is_numeric($month)&&is_numeric($date)&&$month>0&&$month<13&&$date>0&&$date<32 && $year>0 && $year<2100) {
			return true;
		} else {
			return false;
		}
