...
<thead>
	...
	foreach($MemberSubscriberList as $item) {
		...
		echo '<td class="snum">'.preg_replace('/([0-9]{2})([0-9]{3})([0-9]{3})([0-9]*)/', '0$1 $2 $3$4', $item).'</td>';
...
$pagination = pagination_bootstrap('search/ajax_members_list/'.$GroupId, $page, $MemberAmount, 48, 4);
...
.member-paging .pagination {
    float: right;
    margin-bottom: 0;
    margin-top: 10px;
}
.member-paging .paging-details {
    color: #999;
    float: right;
    font-size: 11px;
    font-style: oblique;
