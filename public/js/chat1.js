...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if (jQuery("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				jQuery("#chatbox_"+chatboxtitle).css('right', '200px');
			} else {
...
function chatHeartbeat(){
	...
	jQuery.ajax({
	  url: "http://localhost:81/webproject/selfcare/chat/chat_heart_beat",
	  cache: false,
	  dataType: "json",
	  data:{"csrf_sc_name":jQuery.cookie('csrf_cookie_name')},
...
function closeChatBox(chatboxtitle) {
	...
	restructureChatBoxes();
	jQuery.post("http://localhost:81/webproject/selfcare/chat/chat_heart_beat/close_chat", { chatbox: chatboxtitle,"csrf_sc_name":jQuery.cookie('csrf_cookie_name')} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jQuery.post("http://localhost:81/webproject/selfcare/chat/send_chat", {to: chatboxtitle, message: message,"csrf_sc_name":jQuery.cookie('csrf_cookie_name')} , function(data){
				message = message.replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;");
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+username+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent"><div>'+message+'</div></span></div>');
				jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent").scrollTop(jQuery("#chatbox_"+chatboxtitle+" .chatboxcontent")[0].scrollHeight);
			});
		}
		chatHeartbeatTime = minChatHeartbeat;
...
function startChatSession(){ 
	var param="csrf_sc_name="+jQuery.cookie('csrf_cookie_name');
	jQuery.ajax({
	  url: "http://localhost:81/webproject/selfcare/chat/start_chat_session",
	  cache: false,
	  data:param,
	  dataType: "json",
	  success: function(data) {
